...
//this is main function JNI, searchFace - it should follow the convention as Java_org_opencv_samples_tutorial1_Tutorial1Activity_searchFace
//for example searchFace will be called/used in the activity Tutorial1Activity (org.opencv.sample.tutorial1)
//JNI function is declared: Java_org_opencv_samples_tutorial1_Tutorial1Activity_searchFace (change . -> _)
//JNIEnv* env, jobject thiz - default inputs for all functions (environment and object)
//jstring jDBFolder, jstring jQueryFolder are dbfolder and query folder - our inputs
JNIEXPORT jint JNICALL Java_org_opencv_samples_tutorial1_Tutorial1Activity_searchFace(JNIEnv* env, jobject thiz, jstring jSourceFile, jstring jDBFolder, jobjectArray jCategories, jint jNumFaceResults, jobjectArray jOutputFileNames, jfloatArray jOutputScores);

JNIEXPORT jint JNICALL Java_org_opencv_samples_tutorial1_Tutorial1Activity_searchFace(JNIEnv* env, jobject thiz, jstring jSourceFile, jstring jDBFolder, jobjectArray jCategories, jint jNumFaceResults, jobjectArray jOutputFileNames, jfloatArray jOutputScores) {

	//native string: convert jstring to const char*
	char nDBFolder[1024];
	const char *tmpDBFolder = env->GetStringUTFChars(jDBFolder, JNI_FALSE);
	strcpy(nDBFolder, tmpDBFolder);
	//release
	env->ReleaseStringUTFChars(jDBFolder, tmpDBFolder);

	char nSourceFile[1024];
	const char *tmpSourceFile = env->GetStringUTFChars(jSourceFile, JNI_FALSE);
	strcpy(nSourceFile, tmpSourceFile);
	//release
	env->ReleaseStringUTFChars(jSourceFile, tmpSourceFile);

	//get number of categories
	jsize categoryCount = (*env).GetArrayLength(jCategories);
	char *nCategories[categoryCount];


	for (int i=0; i<categoryCount; i++) {
		//get jString from objectArray
		jstring category = (jstring) (*env).GetObjectArrayElement(jCategories, i);
		const char *tmp = (*env).GetStringUTFChars(category, JNI_FALSE);
		nCategories[i] = (char*) malloc(1024);
		strcpy(nCategories[i], tmp);
		//free - release string
		env->ReleaseStringUTFChars(category, tmp);
	}

	char** outputFileNames = (char**) malloc(jNumFaceResults*sizeof(char*));
	float* outputScores = (float*)malloc(jNumFaceResults*sizeof(float));
	...
	//return the results via the class string and float

	for(int i=0; i<jNumFaceResults; i++)
	{
		env->SetObjectArrayElement(jOutputFileNames, i, env->NewStringUTF(outputFileNames[i]));
		//env->SetObjectArrayElement(jOutputScores, i, env->New(jfloat)outputScores[i]);
		//if (outputScores[i] != 0)
			//retCode = 2;
	}

	env->SetFloatArrayRegion(jOutputScores, 0, jNumFaceResults, outputScores);
	...
	//free outputFileNames, outputScores
